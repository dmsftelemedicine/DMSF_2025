...
<x-app-layout>
    ...
    <script>
        ...
        window.currentConsultationId = root?.dataset.consultationId 
            ? Number(root.dataset.consultationId) || null 
            : null;
        window.currentConsultationNumber = root?.dataset.consultationNumber 
        ...
        document.querySelectorAll('#measurementsTab button[data-bs-toggle="tab"]').forEach(button => {
            button.addEventListener('shown.bs.tab', function(event) {
                const consultationId = event.target.getAttribute('data-consultation-id');
                const consultationNumber = event.target.getAttribute('data-consultation-number');
                ...
                if (document.getElementById('other-lm-vs-tab-pane').classList.contains('active')) {
                    // Dispatch custom event to notify lifestyle measures components
                    const consultationChangeEvent = new CustomEvent('consultationChanged', {
                        detail: {
        ...
        // Also track when the Other LM VS tab becomes active
        document.getElementById('other-lm-vs').addEventListener('shown.bs.tab', function() {
            // Dispatch consultation change event when this tab is activated
            const consultationChangeEvent = new CustomEvent('consultationChanged', {
                detail: {
                    consultationId: window.currentConsultationId,
                    consultationNumber: window.currentConsultationNumber
